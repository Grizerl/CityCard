...
<form action="{{ route('transport.update', $transport['id']) }}" method="post">
  ...
  @method('put')
    <div class="mb-3">
        ...
        <input type="text" class="form-control" name="transport_type" value="{{ old('transport_type', $transport->transport_type) }}">
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" name="transport_number" value="{{ old('transport_number', $transport->transport_number) }}">
    ...
    <div class="mb-3">
        ...
        @foreach ($city as $cities)
                    <option value="{{$cities['id']}}" {{ $cities['id'] == $transport->city_id ? 'selected' : '' }}>
                      {{$cities['name']}}
                    </option>
                  @endforeach
              </select>
    ...
    @endif
  <button type="submit" class="btn btn-primary">Modify Transport</button>
</form>
@endsection